...
import {
  ArrowLeft, Printer, CheckCircle2, XCircle, RotateCcw, X,
} from 'lucide-react'
...
interface RepairReceiptPrintFlowProps {
  repairId:         string
  onClose:          () => void
...
export function RepairReceiptPrintFlow({
  repairId, onClose, successNavItems,
}: RepairReceiptPrintFlowProps) {
  ...
  const origin = typeof window !== 'undefined' ? window.location.origin : 'https://fixitpro.in.th'
  ...
  function handlePrintA4() {
    window.open(`/print/repair/${repairId}?paper=A4`, '_blank')
  }
  ...
  function Header({ onBack }: { onBack?: () => void }) {
    return (
      <div
        className="bg-blue-600 text-white px-4 py-3 flex items-center gap-3 shrink-0"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-base leading-tight">พิมพ์ใบรับซ่อม</p>
          {printer && state === 'preview' && (
            <p className="text-xs text-blue-200 truncate">{printer.name}</p>
          )}
        </div>
        {onBack && (
          <button
      ...
      <div className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center gap-6 px-8">
        ...
        <div className="w-full space-y-2">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full h-12 rounded-2xl bg-blue-600 text-white font-bold text-sm"
          >
    ...
    <div className="fixed inset-0 z-50 bg-white flex flex-col">
      ...
      >
        ...
        <button
          ...
          className="h-14 rounded-2xl bg-blue-600 text-white font-bold text-base flex items-center justify-center gap-2 active:bg-blue-700 disabled:opacity-60"
        >
          {printing ? (
