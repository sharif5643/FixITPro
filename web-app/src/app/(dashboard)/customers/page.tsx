...
const TIER_CONFIG: Record<Tier, { label: string; Icon: React.ElementType; cls: string }> = {
  ...
  NEW:     { label: 'ใหม่',  Icon: Sparkles, cls: 'bg-slate-50 text-slate-500 border-slate-200' },
}
      ...
      {crmStats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">ลูกค้าทั้งหมด</p>
            <p className="text-2xl font-bold text-slate-900 mt-1 tabular-nums">{crmStats.total}</p>
            <p className="text-xs text-slate-400 mt-0.5">{formatThaiMoney(crmStats.totalSpend)} ยอดรวม</p>
          </div>
          <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4">
            <p className="text-[11px] font-semibold text-yellow-600 uppercase tracking-wide flex items-center gap-1">
              <Crown className="h-3 w-3" /> VIP
            </p>
            <p className="text-2xl font-bold text-yellow-800 mt-1 tabular-nums">{crmStats.vip}</p>
            <p className="text-xs text-yellow-600 mt-0.5">ยอดซื้อ ≥ ฿10,000</p>
          </div>
          <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
            <p className="text-[11px] font-semibold text-blue-600 uppercase tracking-wide flex items-center gap-1">
              <Star className="h-3 w-3" /> ประจำ
            </p>
            <p className="text-2xl font-bold text-blue-800 mt-1 tabular-nums">{crmStats.regular}</p>
            <p className="text-xs text-blue-600 mt-0.5">ซื้อ ≥ 3 ครั้ง</p>
          </div>
          <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
            ...
            <p className="text-2xl font-bold text-slate-700 mt-1 tabular-nums">{crmStats.newCount}</p>
            <p className="text-xs text-slate-400 mt-0.5">ยังไม่มีประวัติซ้ำ</p>
          </div>
        </div>
      )}
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div className="rounded-lg bg-slate-50 py-2">
                    <p className="text-[10px] text-slate-400">ซื้อ</p>
                    <p className="font-bold text-sm">{salesCount}</p>
                  </div>
                  <div className="rounded-lg bg-slate-50 py-2">
                    <p className="text-[10px] text-slate-400">ซ่อม</p>
